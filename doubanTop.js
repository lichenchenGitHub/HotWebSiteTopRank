    ...
    var crawlData = function () {
        if (flag) {
            ...
            Promise.map(urlList, function (url, index) {
                //爬取
                return dataDeal.crawllingData(url, conf.doubanTop.dataType).then(function (data) {
                    //数据映射
                    ...
                    return Promise.map(data, function (content, index) {
                        //数据逐条入库
                        return dataDeal.insertOrUpdate(doubanModel, content, url);
                    }).then(function (data) {
                        //写入日志表
